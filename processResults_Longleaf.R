...
run_processScript = function(mu=1, sd=0, beta=0.25, pi=0.3, miss_pct_features = 50,
  ...
  for(i in 1:length(Ns)){for(j in 1:length(Ps)){for(k in 1:length(Ds)){
    ...
    for(m in 1:length(mechanisms)){
      ...
      if(file.exists(fname)){
        ...
        for(s in 1:length(sim_indexes)){
          print("sim_index:"); print(sim_indexes[s])
          ...
          list_res[[s]] = processResults(prefix=prefix,data.file.name = data.file.name, mask.file.name=mask.file.name,
    ...
    for(l in 1:length(perf_metrics)){
      ...
      if(perf_metrics[l] %in% c("predC_L1","predI_L1")){
...
names(args2) = c("N", "P", "D", "data_type_y")

args2$Cy = if(args2$data_type_y =="real"){NULL} else if(args2$data_type_y=="cat"){2}

run_processScript(mu, sd, beta, pi, miss_pct_features,
                  Ns = args2$N, Ps=args2$P, Ds=args2$D, phi0, case, data_type_y=args2$data_type_y, Cy=args2$Cy,
