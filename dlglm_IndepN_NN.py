def dlglm(X,Rx,Y,Ry, covars_r_x, covars_r_y, norm_means_x, norm_sds_x, norm_mean_y, norm_sd_y, learn_r, Ignorable=False, family="Gaussian", link="identity", impute_bs=None,arch="IWAE",add_miss_term=False,draw_miss=True,pre_impute_value=0,n_hidden_layers=2,n_hidden_layers_r=0,h1=8,h2=8,h3=0,phi0=None,phi=None,train=1,saved_model=None,sigma="elu",bs = 64,n_epochs = 2002,lr=0.001,L=20,M=20,trace=False):
  ...
  use_NN_pxm = True ## if all_covars = False, then whether NN is used to model p(xm|xo) (independent)
  ...
  if all_covars:
    ...
    # only missing covars
    if use_NN_pxm: NN_pxm = network_maker(act_fun, n_hidden_layers, np.sum(full_obs_ids), h1, 2*p_miss, True, False).cuda()
    # if use_NN_pxm: NN_pxm = network_maker(act_fun, n_hidden_layers, np.sum(full_obs_ids), h1, int(p_miss+(p_miss*(p_miss+1)/2)), True, False).cuda()
    else: mu_x = torch.zeros(p_miss, requires_grad=True, device="cuda:0");  scale_x = torch.eye(p_miss, requires_grad=True, device="cuda:0")
  ...
  def forward(iota_xfull, iota_x, iota_y, mask_x, mask_y, batch_size, niw):
    ...
    if use_NN_pxm:
      out_NN_pxm = NN_pxm(iota_x[:, full_obs_ids])
      mu_x = out_NN_pxm[..., :p_miss]
      
      scale_x = torch.nn.Softplus()(out_NN_pxm[..., p_miss:(2*p_miss)]+0.001)
      p_x = td.Normal(loc=mu_x, scale=scale_x)
      
      ## MVN?
      # tril_indices = torch.tril_indices(row=int(p_miss), col=int(p_miss), offset=0)
      # scale_x = torch.zeros((p_miss,p_miss)).cuda()
      # print(tril_indices[0])
      # print(tril_indices[1])
      ## PROBLEM HERE: you have to do this for each row of out_NN_pxm[..., p_miss:int(...)].
      ## What then? p_x must be formulated using each cov matrix
      # scale_x[tril_indices[0], tril_indices[1]] = out_NN_pxm[..., p_miss:int(p_miss+(p_miss*(p_miss+1)/2))]
      # print("hi")
      # print(scale_x)
      # p_x = td.multivariate_normal.MultivariateNormal(loc=mu_x, scale.tril=scale_x)  # actually independent Normal: diagonal scale_x
    else:
    ...
    if miss_x and miss_y:     # case 3
      ...
      if miss_x and not miss_y:  # case 1
        ...
        if all_covars:
          ...
          # just missing covars
          if use_NN_pxm:
            logpx = torch.sum(p_x.log_prob(xincluded[:,miss_ids].reshape([M,batch_size,-1])) , 2)
            # logpx = p_x.log_prob(xincluded[:,miss_ids].reshape([M,batch_size]))
            # print(logpx.shape)
          else:
  ...
  if use_NN_pxm: params = params + list(NN_pxm.parameters())
  ...
  if use_NN_pxm: NN_pxm.apply(weights_init)
  ...
  if train==1:
    ...
    for ep in range(1,n_epochs):
      ...
      for it in range(len(batches_x)):
        ...
        if use_NN_pxm: NN_pxm.zero_grad()
    ...
    # plt.xlabel("Epochs")
    # plt.show()
    mu_x=None; scale_x=None # just dummy
    ...
    if use_NN_pxm: saved_model['NN_pxm']=NN_pxm
    ...
    if not use_NN_pxm: all_params = {'x': {'mean':mu_x.cpu().data.numpy(), 'scale':scale_x.cpu().data.numpy()}}
    else: all_params = {}
    ...
    if use_NN_pxm: NN_pxm=saved_model['NN_pxm']
    ...
    if use_NN_pxm: all_params = {'x': {'mean':mu_x.cpu().data.numpy(), 'scale':scale_x.cpu().data.numpy()}}
    else: all_params={}
